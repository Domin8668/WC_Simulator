                ...
                if (_register == null)
                {
                    _register = new RelayCommand(arg =>
                    {
                        ...
                        if (Model.CheckLogin())
                        {
                            ...
                            SecurityAnswer = null;
                            UsernameBorder = 0.7;
                            UsernameWarning = "Podany użytkownik już istnieje";
                            Model.CurrentUserShort.Login = null;
                            return;
                        }
                        Model.CurrentUserShort.Password = NewSecurePassword;
                        Model.CurrentUser.Login = Username;
                        Model.CurrentUser.Password = NewSecurePassword;
                        Model.CurrentUser.Security_question = SelectedSecurityQuestion;
                        Model.CurrentUser.Security_answer = SecureSecurityAnswer;

                        RepositoryUsers.AddUser(Model.CurrentUser);
                        Model.AllUsers.Add(Model.CurrentUser);
                        Model.AllUsersShort.Add(Model.CurrentUserShort);
                        ...
                        NavigationStore.CurrentViewModel = new MessageViewModel(Model, NavigationStore, login, Visibility.Hidden, "Konto zostało utworzone.");
                    },
                    arg => true);
                }
                return _register;
